...
def run_rcon_command(command):
    try:
        # Библиотека rcon возвращает строку. Делаем стрип от лишних пробелов.
        response = rcon(command, host=MINECRAFT_IP, port=RCON_PORT, password=RCON_PASSWORD)
        return response if response else "Команда выполнена (нет ответа от сервера)"
...
async def on_ready():
    ...
    # Установим боту красивый статус активного сервера
    await bot.change_presence(activity=discord.Game(name="на 57 СМП (1.21)"))

# 1. Реакция на упоминание бота (ВМЕСТО реакции на абсолютно любое сообщение)
@bot.event
async def on_message(message):
    if message.author.bot:
        return

    # Бот ответит меню только в том случае, если кто-то тегнет его @ИмяБот-а
...
async def whitelist(interaction: discord.Interaction, nickname: str):
    # Очистка ника от запрещенных символов для безопасности RCON инъекций
    ...
    # Расширили проверку ответов ядра сервера (Paper/Purpur/Spigot)
    ...
    if any(word in rcon_response.lower() for word in success_keywords):
        ...
        await interaction.followup.send(f"❌ Не удалось добавить игрока. Ответ сервера: `{rcon_response}`. Проверьте, включен ли сервер.")
